...
std::cout << "\n" << "\n" << duration << "\n" << "\n";
pid_t pid;
if(duration==.399 && std::stod(getenv("curr_vel"))<std::stod(getenv("max_vel")))
{
	std::string line2=getenv("BUILDER_SCRIPT_PATH");
	
	line2+="/setup-plugins-capt.sh";

	
        myfile << "\n" << "\n" << "\n" <<"Forking to Capt " <<currSpeed << "\n" << "\n" << "\n";
...
else if(duration==.399 && std::stod(getenv("curr_vel"))==std::stod(getenv("max_vel")))
	{
